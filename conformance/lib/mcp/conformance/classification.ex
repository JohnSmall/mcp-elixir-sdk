defmodule MCP.Conformance.Classification do
  ...
  @ours %{
    "server-stateless" => %{
      class: :real_gap,
      why:
        "17 sep-2575 checks, and six distinct defects rather than one — and these " <>
          "six account for all 17, none left over: (a) a request whose _meta omits a " <>
          "required field (the three cases the run actually exercises: no _meta at all, " <>
          "no protocolVersion, no clientCapabilities) is answered HTTP 200 with a normal " <>
          "RESULT and no error object at all, where the SEP requires -32602 and HTTP 400; " <>
          "(b) a request naming an unsupported protocolVersion is answered the same way — " <>
          "HTTP 200, a normal RESULT, NO error object — so there is no supported-versions " <>
          "payload to be mis-shaped: the check reads error.data.supported, finds nothing, " <>
          "and prints its own JSON.stringify(undefined) back (\"data layout does not " <>
          "correlate to active server metrics: undefined\"). That is R3 — server/discover " <>
          "has no version gate — which the MES-58 report carries as a DECISION row for the " <>
          "PO, not a fix row: NAMED AND ROUTED HERE, NOT RESOLVED. The paired " <>
          "sep-2575-http-server-unsupported-version-400 (observed HTTP 200, SEP requires " <>
          "400) is R2, MES-43 gap 3; (c) FIVE checks — the removed methods ping, " <>
          "logging/setLevel, resources/subscribe and resources/unsubscribe, plus the " <>
          "generic unknown/method probe — are answered HTTP 200 where the SEP requires " <>
          "404, and for these five the JSON-RPC code IS already -32601, so only the HTTP " <>
          "status is wrong. That is R2, MES-43 gap 3. initialize is NOT one of them, see " <>
          "(f); (d) the -32021 MissingRequiredClientCapabilityError carries no error.data " <>
          "AT ALL — the capability set is stringified into the error MESSAGE (\"Missing " <>
          "required client capability: sampling (requiredCapabilities: {\"sampling\":{}})\"), " <>
          "so error.data.requiredCapabilities is ABSENT rather than mis-shaped; the " <>
          "harness names an array only because its generic wording explains the schema, " <>
          "and it observed none. That is R5 — a handler error return has no data slot — " <>
          "owned by MES-43 gap 4; the paired sep-2575-missing-capability-http-400 failure " <>
          "on the same error (observed HTTP 200, SEP requires 400) is R2, MES-43 gap 3. " <>
          "(e) a request whose MCP-Protocol-Version header disagrees with " <>
          "_meta.protocolVersion is answered HTTP 200 with no error, where the SEP " <>
          "requires HTTP 400 and -32020 HeaderMismatch — check " <>
          "sep-2575-http-server-header-mismatch-400, observed \"got status 200 with code " <>
          "undefined\". That is R4 — no cross-check of the header against _meta — and R4 " <>
          "is the one root cause here with NO owning ticket. (f) initialize, the sixth " <>
          "check of (c)'s family, is the one that does NOT observe -32601: it observes " <>
          "\"got HTTP 200 and code -32022\", the SDK answering it as an unsupported " <>
          "protocol version rather than as a removed method. So (c)'s \"the code is " <>
          "already right\" would be FALSE of it, and a fixer reading (c) would treat " <>
          "initialize as an HTTP-status-only fix. That is R6, which the MES-58 report " <>
          "carries as a DECISION row for the PO alongside R3: NAMED AND ROUTED HERE, NOT " <>
          "RESOLVED. Its HTTP-200 half is R2. Core server, squarely inside ADR-003's " <>
